...
@st.cache_data
def code_lama_70b(token,prompt,temp,top_p,seed):
...
def display_code_lama_70B(token):
    
    st.markdown("<h1 style=text-align:center;'>Code Llama 70B</h1>", unsafe_allow_html=True)
    # st.write("Code lama 34b hyper params")
    with st.sidebar:
        st.title("Parameters Tuning (70B)")
    ...
    if "message_70b" not in st.session_state:
        st.session_state["message_70b"] =[]

    for msg in st.session_state.message_70b:
    ...
    #     message(f"{result}")
    if prompt:
        st.session_state.message_70b.append({"role":"user","content":prompt})
        ...
        result = code_lama_70b(token,prompt,st.session_state.val,st.session_state.val1,st.session_state.val2)

        st.session_state.message_70b.append({"role":"assistant","content":result})
        with st.chat_message("assistant"):
            st.write(result)
