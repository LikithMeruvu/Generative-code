...
@st.cache_data
def code_lama_7b(token,prompt,temp,top_p,seed):
...
def display_code_lama_7B(token):
    st.markdown("<h1 style=text-align:center;'>Code Llama 7B</h1>", unsafe_allow_html=True)
    # st.write("Code lama 7b hyper params")
    with st.sidebar:
        ...
        st.session_state.val1 = st.slider("Select Top_P", key="slider2", min_value=0.1, max_value=1.0, value=0.1, step=0.1,help = "nucleus sampling probability threshold")
        if st.session_state.val1 > 0.9:
            st.session_state.val1 = 1.0
        st.write('Top_P:', st.session_state.val1)

        st.session_state.val2 = st.slider("Select Seed", key="slider3", min_value=1, max_value=1000, value=42, step=1,help = " ")
    ...
    if prompt:
        ...
        result = code_lama_7b(token,prompt,st.session_state.val,st.session_state.val1,st.session_state.val2)
